connect gingersnap/password@//localhost:1521/XEPDB1
...
create table gingersnap.airport ("id" int primary key, "name" varchar(255));
...
alter table gingersnap.flight add foreign key ("origin_id") references gingersnap.airport("id");
alter table gingersnap.flight add foreign key ("destination_id") references gingersnap.airport("id");