...
create table gingersnap.airport(id int not null primary key, name varchar(255));
...
create table gingersnap.flight(
    ...
    origin_id int foreign key references gingersnap.airport(id),
    destination_id int foreign key references gingersnap.airport(id)
);
